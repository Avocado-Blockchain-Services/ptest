...
- test_init_smoke.py: `_git` (fake marker) → `fake_git_marker`;
  four inline dispatcher-plus-child builders (single-web, api/web persea,
  two single-api) → `monorepo` fixture (`parent=tmp_path, name=None`,
  persea root bytes preserved via `root_toml`; payload files via
  `support.write_file`); `_pytest_toml_with_setup` (3 setup-gating sites +
  executability-agreement site) and `_vitest_toml_with_setup` deleted →
  `support.write_ptest_toml` (`args=()`, `full_args=()` explicit;
  project_ids `aa`/`bb`/`cc`/`dd`/`ee`/`ff` preserved);
  `_npm_locked_vitest_repo` now composes `fake_git_marker` /
  `write_file` / `write_ptest_toml`.
...
- `test_selection.py::test_choose_plan_is_pure_without_subprocess_or_test_imports`
  ...
  first). Fixed inside the owned file with no weakened assertion: the
  original predicate (`name in {"pytest", "conftest"} or
  name.startswith("test_")`, exact-name match, no already-imported
  exemption) is preserved, and the patch is scoped to the planning call
  via `monkeypatch.context()` — `plan = choose_plan(...)` inside the
  block, `assert plan.execution == "none"` outside — so the bridge's
  call-phase lazy `from pytest import UsageError` runs unpatched. The
  `sys.modules` exemption/diff machinery is deleted. T1 may want to
  harden the bridge's lazy import; no src change was needed here.
...
- Commit `57bbca0` ("revert out-of-lane files") deleted this ledger and
  reverted the frozen `.pipeline/design.md`, `.pipeline/shared-bundle.patch`
  and the shared bundle (`tests/ng/conftest.py`, `tests/ng/support.py`,
  `pyproject.toml`, `.ptest.toml`), leaving the branch self-inconsistent
  (migrated files import bundle helpers absent from HEAD) and the merge
  base (`aa34d2a`) rewriting frozen orchestrator artifacts on merge.
  Recovery: all seven paths restored from `f5602d5` (pre-revert); the
  gate/owned-list conflict behind the revert is left for the orchestrator
  to decide rather than re-reverting frozen files.
