...
_QUEUE_DEFAULT_S = 1800.0
_MAX_QUEUE_S = 86400.0
_MAX_PENDING = 256
_MAX_TERMINAL = 256
_ACTIVE = frozenset({
...
_SCHEMA_TABLES = frozenset({"domain", "jobs", "job_resources"})
...
def _parse_limits(data: object, *, fixture: bool) -> tuple[int, int, int | None]:
    ...
    slots = _check_int(data["max_slots"], lo=1, hi=64, label="max_slots")
    jobs = _check_int(data["max_jobs"], lo=1, hi=64, label="max_jobs")
    if jobs > slots:
...
def _load_config_limits(domain: DomainPaths, *, create_missing: bool) -> tuple[int, int, int | None]:
    if domain.fixture:
        ...
        } or data.get("version") != 1 or data.get("fixture") is not True
...
def _marker_object(domain: DomainPaths, ledger: Path) -> dict:
    stamp = os.lstat(ledger)
    return {
        "domain_id": domain.domain_id or secrets.token_hex(16),
        "ledger_device": stamp.st_dev,
        "ledger_inode": stamp.st_ino,
...
def _read_state_marker(domain: DomainPaths) -> dict:
    ...
    if value["protocol_version"] != _PROTOCOL_VERSION or value["schema_version"] != _SCHEMA_VERSION:
    ...
    try:
        ...
        with conn:
            ...
            conn.execute(
                ...
                (domain_id, _SCHEMA_VERSION, _PROTOCOL_VERSION,
                 ledger_stamp.st_dev, ledger_stamp.st_ino, boot,
                 limits[0], limits[1], limits[2], 0),
            )
        files.create_exclusive(root, marker.name,
                               json.dumps({
                                   "domain_id": domain_id,
                                   "ledger_device": ledger_stamp.st_dev,
                                   "ledger_inode": ledger_stamp.st_ino,
                                   "protocol_version": _PROTOCOL_VERSION,
                                   "schema_version": _SCHEMA_VERSION,
...
def _ensure_state(domain: DomainPaths) -> None:
    ...
    if marker_exists != ledger_exists:
        _fail("coordinator-corrupt", "coordinator marker and database are inconsistent")
    if marker_exists:
        _validate_existing(domain)
        return
    with _bootstrap(root):
        marker_exists = marker.exists() or marker.is_symlink()
        ledger_exists = ledger.exists() or ledger.is_symlink()
        if marker_exists != ledger_exists:
            _fail("coordinator-corrupt", "coordinator marker and database are inconsistent")
...
def _validate_existing(domain: DomainPaths) -> None:
    root = Path(domain.root)
    files.validate_private_file(Path(domain.root) / "bootstrap.lock")
    marker = _read_state_marker(domain)
    files.validate_private_file(Path(domain.ledger))
    ledger_stamp = os.lstat(domain.ledger)
    if (marker["ledger_device"], marker["ledger_inode"]) != (ledger_stamp.st_dev, ledger_stamp.st_ino):
        _fail("unsafe-path", "coordinator database identity changed")
    if domain.domain_id is not None and marker["domain_id"] != domain.domain_id:
        _fail("unsafe-path", "coordinator domain identity changed")
    try:
        conn = storage.open_database(root, Path(domain.ledger).name, max_bytes=_DB_MAX_BYTES)
        _validate_schema(conn)
        row = conn.execute("SELECT * FROM domain WHERE singleton=1").fetchone()
        if row is None:
            _fail("coordinator-corrupt", "coordinator identity row is missing")
        names = [item[1] for item in conn.execute("PRAGMA table_info(domain)").fetchall()]
        values = dict(zip(names, row))
        if (values.get("domain_id") != marker["domain_id"]
                or values.get("ledger_device") != ledger_stamp.st_dev
                or values.get("ledger_inode") != ledger_stamp.st_ino
                or values.get("schema_version") != _SCHEMA_VERSION
                or values.get("protocol_version") != _PROTOCOL_VERSION):
            _fail("protocol-mismatch", "coordinator identity does not match its marker")
        if domain.fixture:
            fixture_limits = _load_config_limits(domain, create_missing=False)
            if fixture_limits != (
                    values.get("config_slots"), values.get("config_jobs"),
                    values.get("config_memory")):
...
def _open_state(domain: DomainPaths, *, create: bool) -> tuple[sqlite3.Connection, dict]:
    if create:
        _ensure_state(domain)
    else:
        _prepare_root(domain) if domain.fixture else _validate_domain_argument(domain)
        root = Path(domain.root)
        marker = _state_marker(domain)
        ledger = Path(domain.ledger)
        if not marker.exists() or not ledger.exists():
            if marker.exists() or ledger.exists():
                _fail("coordinator-corrupt", "coordinator marker and database are inconsistent")
            _fail("state-unavailable", "coordinator has not been initialized")
        _validate_existing(domain)
    conn = storage.open_database(Path(domain.root), Path(domain.ledger).name,
                                 max_bytes=_DB_MAX_BYTES)
    conn.row_factory = sqlite3.Row
    _validate_schema(conn)
    try:
        row = conn.execute("SELECT * FROM domain WHERE singleton=1").fetchone()
        columns = [item[1] for item in conn.execute("PRAGMA table_info(domain)").fetchall()]
        info = dict(zip(columns, row)) if row is not None else {}
        if not info:
            _fail("coordinator-corrupt", "coordinator identity is missing")
        current_boot = platform.boot_identity()
        if current_boot != info["boot_id"]:
            _fail("ownership-uncertain", "kernel boot identity changed")
        return conn, info
    except Exception:
        conn.close()
        raise
...
def _limits_changed(new: tuple[int, int, int | None], old: tuple[int, int, int | None]) -> tuple[bool, bool]:
    decreased = new[0] <= old[0] and new[1] <= old[1]
    if old[2] is not None and new[2] is None:
        decreased = True
    elif old[2] is not None and new[2] is not None and new[2] <= old[2]:
        decreased = decreased and True
    elif old[2] is None and new[2] is not None:
        decreased = False
    elif old[2] is not None and new[2] is not None and new[2] > old[2]:
        decreased = False
    increased = new[0] >= old[0] and new[1] >= old[1] and not decreased
...
def _refresh_limits_locked(conn: sqlite3.Connection, domain: DomainPaths, info: dict) -> dict:
    ...
    if requested == old:
        return info
    decreased, increased = _limits_changed(requested, old)
    if increased and (_active_count(conn) or _pending_count(conn)):
        return info
    if not decreased and not increased:
        _fail("invalid-config", "machine limits changed incompatibly")
...
def _resources_available(conn: sqlite3.Connection, request: dict, slots: int,
    ...
    if budget is not None:
        used = sum(int(row[3]) for row in active if row[3] is not None)
        if reserved is None or used + reserved > int(budget):
            return False
    return True


def _observe_owner(row: sqlite3.Row | dict) -> ProcessIdentity | None:
    try:
        return platform.process_identity(int(row["owner_pid"]))
...
def _reconcile_locked(conn: sqlite3.Connection, now: float) -> None:
    rows = conn.execute("SELECT * FROM jobs WHERE state IN (%s) ORDER BY sequence"
                        % ",".join("?" for _ in _ACTIVE), tuple(_ACTIVE)).fetchall()
    for row in rows:
        owner = _observe_owner(row)
        state = row["state"]
        if state == "GRANTED":
            if owner is None or not _same_identity(owner, row):
                conn.execute(
                    "UPDATE jobs SET state='CANCELLED',phase='complete',reason_code=?,reason_message=? WHERE run_id=?",
                    ("ownership-uncertain", "grant owner disappeared before guard registration", row["run_id"]),
                )
            continue
        pgid = row["guard_pgid"] if row["guard_pgid"] is not None else row["owner_pgid"]
        observation = platform.probe_group(int(pgid))
        if observation.exists is True:
            continue
        if observation.exists is None or not observation.permission:
            conn.execute(
                "UPDATE jobs SET state='UNCERTAIN',reason_code=?,reason_message=? WHERE run_id=?",
                ("ownership-uncertain", "process-group ownership could not be proven", row["run_id"]),
            )
            continue
        # An absent group is useful evidence only after the owner is gone.
        # A live finalizer may still be committing the outcome.
        if state == "FINALIZING" and owner is not None:
            continue
        if owner is None:
            conn.execute(
                "UPDATE jobs SET state='RELEASED',phase='complete',reason_code=?,reason_message=? WHERE run_id=?",
                ("ownership-uncertain", "lease released incomplete after proven owner absence", row["run_id"]),
            )
        else:
            conn.execute(
                "UPDATE jobs SET state='UNCERTAIN',reason_code=?,reason_message=? WHERE run_id=?",
                ("ownership-uncertain", "process-group absence lacked finalization proof", row["run_id"]),
            )


def _deadline_for(request: AdmissionRequest, now: float) -> float:
    if request.deadline == 0:
        return now + _QUEUE_DEFAULT_S
    if request.deadline < now:
        return request.deadline
    if request.deadline - now > _MAX_QUEUE_S:
...
def enqueue(domain: DomainPaths, request: AdmissionRequest) -> Ticket:
    ...
    try:
        ...
        if _pending_count(conn) >= _MAX_PENDING:
...
def _grant_queued_locked(conn: sqlite3.Connection, info: dict, now: float) -> None:
    ...
    for row in rows:
        if now >= row["deadline"]:
            conn.execute(
                "UPDATE jobs SET state='CANCELLED',phase='complete',reason_code=?,reason_message=? WHERE run_id=?",
                ("queue-timeout", "admission queue deadline expired", row["run_id"]),
            )
            continue
        requested = {
...
def _grant_from_row(row: sqlite3.Row | dict, domain_id: str) -> Grant | None:
    if row["nonce"] is None or row["generation"] is None or row["slots"] <= 0:
        return None
    return Grant(
...
def register_guard(domain: DomainPaths, grant: Grant, guard: ProcessIdentity) -> bool:
    ...
    try:
        ...
        valid = (row is not None and row["state"] == "GRANTED"
                 and row["nonce"] == grant.nonce and row["generation"] == grant.generation
                 and row["slots"] == grant.slots and row["memory_estimate"] == grant.memory_estimate_mb
                 and row["reserved_memory"] == grant.reserved_memory_mb
                 and info["domain_id"] == grant.domain_id)
        if valid:
            conn.execute(
...
def _lease_view(row: sqlite3.Row | dict, now: float) -> LeaseView:
    ...
    queue_wait = 0.0 if row["grant_time"] is None else max(0.0, row["grant_time"] - row["enqueue_time"])
...
def reconcile(domain: DomainPaths) -> tuple[LeaseView, ...]:
    """Reconcile recoverable ownership facts and return a typed status view."""
    try:
        conn, _info = _open_state(domain, create=False)
    except Problem as exc:
        if exc.code == "state-unavailable":
            return ()
        raise
    ...
    try:
        ...
        result = tuple(_lease_view(row, _now()) for row in rows)
        ok = True
        return result
    finally:
        _finish_transaction(conn, ok)
...
def finish(domain: DomainPaths, grant: Grant, proof: QuiescenceProof,
    ...
    try:
        _begin(conn)
        row = conn.execute("SELECT * FROM jobs WHERE run_id=?", (grant.run_id,)).fetchone()
        valid = (row is not None and row["state"] in {"RUNNING", "DRAINING", "FINALIZING"}
                 and row["nonce"] == grant.nonce and row["generation"] == grant.generation
                 and info["domain_id"] == grant.domain_id
                 and proof.run_id == grant.run_id and proof.generation == grant.generation
                 and proof.pgid == (row["guard_pgid"] or row["owner_pgid"])
                 and proof.group_absent and not proof.escaped_survivors)
        if not valid:
            if row is not None:
                conn.execute(
                    "UPDATE jobs SET state='UNCERTAIN',reason_code=?,reason_message=? WHERE run_id=? AND state NOT IN ('RELEASED','CANCELLED')",
                    ("ownership-uncertain", "finalization lacked a complete quiescence proof", grant.run_id),
                )
                commit_on_error = True
            _fail("ownership-uncertain", "finalization proof did not establish safe release")
        if final.status.value == "passed" and final.committed and final.source_valid:
            owner = _observe_owner(row)
            if owner is None or not _same_identity(owner, row):
                conn.execute(
                    "UPDATE jobs SET state='UNCERTAIN',reason_code=?,reason_message=? WHERE run_id=?",
                    ("ownership-uncertain", "dead owner cannot promote provisional success", grant.run_id),
                )
                commit_on_error = True
                _fail("ownership-uncertain", "dead owner cannot promote provisional success")
        conn.execute(
...
__all__ = ["enqueue", "poll", "register_guard", "reconcile", "finish"]
