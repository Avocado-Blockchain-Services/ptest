...
Status: frozen proposal for the existing Opus design-review gate. Documentation only; no implementation, test, or capability qualification is claimed.
...
The guard reaps each direct attempt child and emits its facts before the next ready event. Supported bridge lifecycle joins/reaps attempt-owned services before terminal completion. If known descendants remain live/unknown, or terminal evidence cannot establish that lifecycle, operations stops; a ready frame is never a quiescence proof. No capacity is released between attempts.

Operations validates each ready event against the expected phase/facts order and matches the preceding report to its allocated run/nonce/attempt/report binding. For shadow/probe it rechecks current config, source and relevant environment identity against the comparison input immediately before every continue; it also checks signal state immediately before sending. Ordinary single-run callbacks preserve their existing source-evidence requirements: an execution-only command is not newly required to supply selection compatibility, and an initial advanced full run captures inputs before native runtime qualification. The first gate covers queue/setup invalidation. A change before any tests may cause a rebuilt full plan in an ordinary automatic run; a requested shadow must obtain another eligible selected/full pair or return `2`/`unsupported-capability` without running a substitute comparison. After an attempt has run, changed/unknown inputs stop the compound operation.
...
| Timeout, incomplete/invalid report, missing planned cases, missing handoff or unavailable identity | Incomplete; no next attempt. A timed-out full run is not a confirmed or suspected omitted-test miss without conclusive matching evidence. | No new divergence classification; retain existing quarantine and required failure/full-gate obligations. |
...
This task never labels a divergence `confirmed-miss` from exit-code contrast alone. Explicit focused reproduction is a separate caller-requested scoped run using the retained failing identity and input, with every attempt visible. The bad-policy sensitivity fixture proves reproducibility in controlled acceptance evidence; product quarantine remains conservative even if the caller does not reproduce. Shadow itself has no hidden reproduction/retry phase. Shadow results do not seed baselines; the existing successful clean `Mode.FULL` publication path remains the sole baseline writer.
...
|---|---|---|
| G1 / T12a | Two real guarded miniature attempts use ordered ready/decision/facts and distinct tokens; ordinary single runs still preserve streams/exits. | Hold the second decision with a pipe barrier: its child marker cannot appear. Send stop after source invalidation and prove it never appears. |
...
| G3 / T12a | Cooperative cancellation drains and proven absence releases within existing fixture targets. | EOF, parent-only SIGKILL, SIGINT/SIGTERM while waiting, incomplete frame, controller stall and gate deadline never start the next entry or release live/uncertain work. Neighbor group sentinel survives. |
...
| S1 / T12c | `a001` exit 23 followed by full `a002` exit 0 or 1 still executes both and returns 23. | No stop solely on ordinary selected failure, no later green exit, no same-run clearing of its failure obligation. |
...
The **S2 sensitivity fixture** is a small committed Git pytest/Vitest project with at least three test files and two independent source groups. A reviewed, syntactically valid but intentionally wrong mapping links `src/shared` only to `tests/alpha`; `tests/beta` also consumes it. Start from a real complete passing full baseline under that same bad mapping, then change only `src/shared`: alpha's assertions remain true, beta's real assertion fails. Full-ratio remains above the selected fraction, with no always/full trigger making selection effectively full. Assert the actual emitted selected files exclude beta, selected executes/passes, full executes/fails beta, `selection-shadow-quarantine` persists, and the next plain automatic plan widens to full. Repeat the omitted beta through an explicit scoped ptest run to demonstrate fixture reproducibility. Never edit policy between baseline and failing comparison, since that must correctly force full.

For recovery, correct the mapping to include both alpha and beta, fix the failing source, commit, and obtain a new clean full baseline while quarantine remains. Make a benign narrow `src/shared` change whose selected alpha+beta fraction is below full ratio and run explicit shadow. Both pass; the exact old selector row clears and unrelated injected health markers in a separate negative case do not. The source fix alone without mapping/compatibility correction must fail the recovery predicate. Perform this sensitivity/recovery with each profile advertised as selectable.
...
There is no unresolved product choice preventing this amendment. The new protocol version, 30-second decision watchdog, code-specific quarantine storage/API and three-step ownership transfer are proposed implementation decisions requiring the existing Opus review before builders consume them. Availability of the exact native profile/platform evidence is an implementation gate, not an ambiguity to paper over. No remote execution, model call, deployment, live installation, publication or source upload is authorized.
