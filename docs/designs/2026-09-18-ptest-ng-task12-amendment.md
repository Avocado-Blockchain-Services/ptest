...
Operations validates each ready event against the expected phase/facts order and matches the preceding report to its allocated run/nonce/attempt/report binding. Immediately before continuing it checks signal state and validates identity against the mode-specific reference below. Ordinary single-run callbacks preserve their existing source-evidence requirements: an execution-only command is not newly required to supply selection compatibility, and an initial advanced full run captures inputs before native runtime qualification.

| Request at `a001` | Frozen identity reference |
|---|---|
| Automatic-selected or shadow | Compare the current required input digest with the admitted selected plan's `input_digest` (the fixed selected/full comparison digest for shadow), together with the config/scope/relevant environment resolution bound to that plan. Queue/setup edits that change this reference invalidate it. |
| Automatic-full, explicit full or explicit scoped | Compare with the retained bounded input/config/environment capture made after admission and before guard launch; the scope must still match the admitted resolution. Do not claim content-delta detection before that capture, or replace it at the gate. |
| Probe | Compare config, resolved scope and relevant environment with the resolution used for admission; reject drift or unknown required identity. The bounded content capture at `a001` is the **first** content reference, not a comparison with admission-time content. Establish it once for later gates/final verification; no content-delta detection across the queue/setup interval is claimed. |

For shadow and probe, every later gate compares required content/config/environment identity with its fixed comparison reference; probe additionally checks the verified runtime identity established by the first serial report. Missing/unknown required identity fails closed even when no earlier content reference exists. None of these checks executes native discovery outside the admitted guard.

**First-gate rule:** if the comparison defined above observes a change, or cannot establish required identity at `a001`, send stop with `changed-during-run` for observed change or `unknown-input` for unknown required identity. Do not rebuild the plan, replace prepared commands, widen scope, alter resource claims or continue under that grant. Complete the existing drain/absence/finalization/release path, with zero test launches. The exact outcomes are:
...
### Closed reason-code additions (T12a)

T12a adds **exactly these five values** to `contracts.REASON_CODES`. `execution-timeout` already occurs as a private guard problem but is not in the current public allowlist; the other four are new. Existing public reason fields carry these codes without adding fields or changing the public schema version. The generated public reason schema already describes `code` as a string; do not invent a second reason catalog or add a reason field to `AttemptResult`. Implement and test the closed enum/public projection before T12b/T12c consumers use it.

| Added code | Intended public use | Intended private use |
|---|---|---|
| `execution-timeout` | Run reason for expiry of an execution or compound deadline; affected attempt status/exit remains in `AttemptResult`. Incomplete, ptest-origin `70` unless an earlier nonzero must survive. Never use it for queue expiry or the decision watchdog. | Guard execution-deadline problem and the already enumerated `attempt-decision.action="stop"` reason when the execution budget is exhausted. It does not authorize another launch. |
| `attempt-decision-timeout` | Run reason when a ready gate receives no valid decision by its deadline; incomplete, ptest-origin `70` absent an earlier nonzero. | Guard/controller diagnostic for the expired pending gate. It is **not** an additional allowed decision-stop reason, control event or synthetic runner-facts message for an unstarted attempt. |
| `selection-shadow-quarantine` | History/selection limitation and automatic-full planning reason while the dedicated selector quarantine row exists; a diagnostic eligible shadow can retain it as context. | Exact `SelectionQuarantine.code` and stored row discriminator, owned only by the typed history API; never a guard decision-stop reason or generic health marker. |
| `probe-no-conflict-observed` | Run reason only for a complete passing probe sample; reports an observation, not certified isolation or selection/full-gate eligibility. | Controller classification from authenticated complete attempt evidence; no guard/bridge verdict or decision-stop reason. |
| `probe-conflict-observed` | Run reason only for the conclusive serial-pass/reproducible-parallel-failure differential in §7; preserve the first failing runner outcome. Arbitrary/incomplete failures do not qualify. | Controller classification from authenticated differential evidence; no guard/bridge verdict or decision-stop reason. |

All other reasons used here are already in `REASON_CODES`; preserve their existing names and meanings. In particular `changed-during-run`, `unknown-input`, `unsupported-capability`, `ownership-uncertain`, `report-invalid`, `incomplete-inventory` and `state-unavailable` are reused, not added. Comparison verdicts (`matched`, `suspected-miss`, `unclassified-divergence`, `incomplete`) are typed internal values, not new reason codes. Existing private transport problems such as `control-unavailable` remain private and use the existing safe public projection; do not expand the reason allowlist to every private problem. The decision-stop subset remains exactly the seven codes in §2.
...
| T12a tests | `tests/ng/test_contracts.py`, `tests/ng/test_guard.py`, `tests/ng/test_operations.py`, `tests/ng/test_history.py`, `tests/ng/test_selection.py`: closed contracts, real-process G1–G3, operations first-gate outcomes, H1–H2 marker/evidence preservation and pure quarantine-aware planning. Exact owned fixtures and exclusions are frozen below. |
...
### T12a repair ownership correction

The Opus-blocked candidate `7a7c7a0` was built under a narrower brief that excluded `src/ptest/selection.py`, `tests/ng/test_selection.py` and all guard fixtures. That restriction cannot deliver the already-approved T12a boundary. For the repair, the owner retains the six source/generated paths and five test modules listed above, including selection's pure `choose_shadow_plans`/quarantine-aware ordinary fallback work; those responsibilities must not be deferred to T12b/T12c or counted complete through record-only tests. The corrected brief must transfer these paths explicitly before repair implementation. This correction changes ownership only, not runtime contracts or the T12a → T12b → T12c order.

T12a also owns exactly these existing fixture files for protocol compatibility and its required lifecycle regressions; ownership is file-specific, not the entire fixture directories:

| Fixture | Permitted purpose and evidence |
|---|---|
| `tests/ng/fixtures/processes/guard_client.py` | Independent admission/controller cancellation owner. Replace protocol-1 control emission with the frozen guard protocol 2 so group-signal/cancellation cases exercise the intended boundary. Scheduler/domain protocol remains unchanged. |
| `tests/ng/fixtures/processes/guard_driver.py` | Test-only barriers, fake monotonic clock and boundary-failure injection around the real guard. Support held decisions, EOF/cancellation/deadline and preceding-phase quiescence cases without replacing the guard or scheduler. |
| `tests/ng/fixtures/processes/guard_workload.py` | Local IPC-controlled child/descendant workloads and launch markers for G1–G3, including live/unknown preceding work and neighbor preservation. Keep bounded cleanup; no external resources or timing-only launch proof. |
| `tests/ng/fixtures/command/guard_faults.py` | Operations tests' real-guard fault injection. Protocol-2 frames must reach the intended wrong-run/nonce/order/handoff boundary instead of failing incidentally as obsolete protocol 1; extend only for the frozen first-gate/lifecycle regressions. |

`tests/ng/conftest.py` and `tests/ng/support.py` remain unowned and unchanged. Test-local helpers belong in the five owned modules or four named fixtures; reuse shared helpers read-only, without weakening their assertions or adding a second shared harness. No other process/command fixture, adapter fixture, source/config/scheduler module, CLI, dependency or packaging path is transferred. The existing report paths remain unchanged. The repair must retain the full real-subprocess G1–G3, H1–H2 marker/evidence and operations first-gate acceptance, with honest scoped evidence across all five test modules; mocks, skipped assertions, record-only tests or a four-module run cannot discharge the omitted obligations. This docs-only correction runs no tests and changes no implementation/test file.
...
A probe does not require enabled selection or a full baseline. At its first gate, config/scope/relevant environment must match the admission resolution, and bounded content identity must be known. This is the first content capture: it establishes the fixed input reference for later gates/final verification, without claiming to detect content edits since admission. The first serial report establishes verified runtime identity using the same initial/terminal observation rule as an initial full run. Later attempts must match both fixed input and verified runtime identities. Unknown content identity or missing runtime observations stop the comparison; no synthetic baseline is created to enable a probe.
...
|---|---|---|
| G1 / T12a | Two guarded synthetic attempts use ordered ready/decision/facts and distinct tokens; custom exit 23 followed by 0/1 preserves 23 at the guard/controller layer. Setup facts use `(setup,a001)` before `(execution,a001)`. | Hold a decision with a pipe barrier: its child marker cannot appear. Invalidate at a001 against each mode's §2 reference and assert exact ordinary/shadow/probe outcomes, no same-grant replan and zero launches. Probe's first content capture must not fabricate admission-time delta evidence; unknown required identity still stops it. This custom-code case is not native selector qualification. |
