...
def _payload_for(packet, rows=None, findings="auto", score="compute"):
    if rows is None:
    ...
    if score == "compute":
        ...
        score = None if na_count == len(rows) else _score(
...
def test_parse_assessment_accepts_valid_single_child(tmp_path):
    ...
    rows = [_row(packet, row_id) for row_id in EXPECTED_IDS]
    raw = C.encode_public_document(
        "agent-assessment", _payload_for(packet, rows=rows))
        ...
        with pytest.raises(C.Problem):
            AA.parse_assessment(
                _envelope_bytes(_payload_for(
                    ...
                    score=None if len(rows) != 11 else "compute")),
                packet)
...
def test_parse_assessment_accepts_affirmative_not_applicable(tmp_path):
    ...
    rows[3] = _row(
        ...
        rationale=(f"Affirmative: {excerpt.path} holds only a constant, "
                   "so no database ownership rule applies."),
        evidence=[_citation_for(packet)])
    raw = C.encode_public_document(
        "agent-assessment", _payload_for(packet, rows=rows))
    child = AA.parse_assessment(raw, packet)
